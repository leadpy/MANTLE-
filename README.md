...
```bash
cd  ~
git clone git@github.com:leadpy/athostec.io.git
```

Na pasta backend, crie o arquivo .env:

```bash
cp athostec/backend/.env.example athostec/backend/.env
nano athostec/backend/.env
```
...
pm2 start dist/server.js --name athostec-backend
```
...
```bash
sudo nano /etc/nginx/sites-available/athostec-backend
```
...
```bash
sudo nano /etc/nginx/sites-available/athostec-frontend
```
...
server {
  ...
  root /home/user/athostec/frontend/dist/pwa; # caminho da pasta dist/pwa
...
```bash
sudo ln -s /etc/nginx/sites-available/athostec-backend /etc/nginx/sites-enabled
sudo ln -s /etc/nginx/sites-available/athostec-frontend /etc/nginx/sites-enabled
```
...
```bash
#!/bin/bash
echo  "Atualizando athostec, aguarde..."
cd  ~
cd athostec
git pull
cd backend
